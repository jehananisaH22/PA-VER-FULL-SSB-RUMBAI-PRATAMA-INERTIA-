...
function buildMonthlyMeetingSlots(yearValue, monthValue, schedules = [], category = "all", mode = "routine") {
  ...
  selectedSchedules.forEach((schedule) => {
    const scheduleDate = parseDateInputValue(schedule.date);
  ...
  return Array.from(new Map(
    ...
    map((slot) => [`${slot.date}-${slot.time}-${slot.scheduleId || ""}`, slot])
...
function CoachPerformanceDatePicker({ value, onChange, ariaLabel, allowedWeekdays = null, allowedDates = null }) {
  const [isOpen, setIsOpen] = useState(false);
  const rootRef = useRef(null);
  ...
  const isDateAllowed = (date) => {
    ...
    const matchesDates = !Array.isArray(allowedDates) || allowedDates.includes(dateValue);
    const matchesWeekdays = !Array.isArray(allowedWeekdays) || allowedWeekdays.includes(date.getDay());
    return matchesDates && matchesWeekdays;
  };
  ...
  const completedPerformanceScheduleIds = useMemo(() => {
    ...
    trainingSchedules.forEach((schedule) => {
      ...
      const isComplete = schedulePlayers.every((player) =>
        localHistory.some((item) =>
          Number(item.studentId) === Number(player.id) &&
          (item.scheduleId || "") === schedule.id &&
          item.date === scheduleDateSlash &&
          scoreFields.every((field) => hasScoreValue(item[field]))
        )
      );
